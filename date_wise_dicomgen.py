"""
Longitudinal Multi-Modality Anatomical DICOM Phantom Generator
==============================================================
Generates CT (axial) AND MR (sagittal T1) series for the SAME patient
across different study dates (simulating longitudinal / follow-up imaging).

Key design:
  • One shared Patient ID across every series and modality
  • Each series gets its own unique StudyInstanceUID + StudyDate
  • Study dates are spread across configurable months / years
  • CT  → axial cross-sections,  HU signed 16-bit
  • MR  → sagittal T1-weighted,  signal 0-1023 unsigned 16-bit

Usage:
    pip install pydicom numpy scipy
    python create_body_dicom_longitudinal.py
...
# ──────────────────────────────────────────────────────────────────────────────
# Study-date pool: spread across ~3 years with realistic intervals
# ──────────────────────────────────────────────────────────────────────────────

def generate_study_dates(n: int, start_year: int = 2021) -> list[tuple[str, str]]:
    """
    Return n (date_str, time_str) pairs sorted in chronological order.
    Dates are spread across 'n' different months, at least 1 month apart,
    spanning up to 3 years from start_year.
    """
    ...
    while len(dates) < n:
        # Random offset: 0 – 36 months from base
        delta_months = random.randint(0, 36)
        year  = base.year  + (base.month - 1 + delta_months) // 12
        month = (base.month - 1 + delta_months) % 12 + 1
        key   = (year, month)
        if key in used_months:
            continue
        used_months.add(key)
        day = random.randint(1, 28)
        d   = datetime.date(year, month, day)
        t_h = random.randint(7, 18)
        t_m = random.randint(0, 59)
        t_s = random.randint(0, 59)
        dates.append((d.strftime("%Y%m%d"),
                      f"{t_h:02d}{t_m:02d}{t_s:02d}"))
...
# ──────────────────────────────────────────────────────────────────────────────
# DICOM file writer  (generic, handles both modalities)
# ──────────────────────────────────────────────────────────────────────────────

def write_dicom(filepath: str,
    ...
    meta.ImplementationVersionName  = "BodyPhantom_Long_1.0"
    ...
    ds.StudyDescription       = (
        f"{modality} Body Phantom – {study_date[:4]}-{study_date[4:6]}-{study_date[6:]}"
    )
    ...
    ds.SeriesDescription  = (
        f"{modality} {pm['plane']}  {study_date[:4]}-{study_date[4:6]}  Ser{series_number}"
    )
...
# ──────────────────────────────────────────────────────────────────────────────
# Per-modality series generator
# ──────────────────────────────────────────────────────────────────────────────

def generate_modality_series(
    modality: str,
    study_date: str,
    study_time: str,
    patient_id: str,
    patient_name: str,
    output_dir: str,
    series_index: int,       # 1-based global counter for display
    total_series: int,
    image_size: int = 512,
    ...
    """
    Generate one DICOM series (one modality, one date) and save to disk.
    Returns the folder path of the written series.
    """
    pm            = _PLANE_META[modality]
    study_uid     = generate_uid()
    series_uid    = generate_uid()
    series_number = random.randint(100, 999)
    num_sl        = num_slices or random.randint(10, 15)

    date_fmt  = f"{study_date[:4]}-{study_date[4:6]}-{study_date[6:]}"
    folder    = os.path.join(
        output_dir,
        f"Series_{series_index:02d}_{modality}_{pm['plane']}_{date_fmt}_S{series_number}"
    )
    ...
    for sl in range(num_sl):
        ...
        else:   # MR sagittal: centred at x=0
            half_span = (num_sl - 1) * slice_thickness / 2.0
            slice_loc = round(-half_span + sl * slice_thickness, 3)

        pixels = gen(sl, num_sl, image_size, image_size)
        ...
        print(f"    slice {sl + 1:>2}/{num_sl}  →  {os.path.basename(filepath)}", end="\r")

    print(f"    ✓  {num_sl} slices saved  →  {folder}          ")
    return folder
...
def create_longitudinal_study(
    output_dir:          str   = "body_dicoms_longitudinal",
    ct_series_count:     int   = 3,   # number of CT sessions (different dates)
    mr_series_count:     int   = 3,   # number of MR sessions (different dates)
    slices_per_ct:       int | None = None,   # None → random 10-15
    slices_per_mr:       int | None = None,   # None → random 10-15
    image_size:          int   = 512,
    slice_thickness:     float = 5.0,
    start_year:          int   = 2021,
):
    """
    Generate CT (axial) + MR (sagittal) series for ONE patient across
    multiple study dates.

    Directory layout:
        <output_dir>/
            Series_01_CT_AXIAL_2021-03-10_S427/
                slice_0001.dcm  …
            Series_02_CT_AXIAL_2022-01-05_S183/
                slice_0001.dcm  …
            Series_03_MR_SAGITTAL_2021-09-22_S614/
                slice_0001.dcm  …
            …

    All series share the SAME PatientID / PatientName.
    Each series has its OWN StudyInstanceUID + StudyDate.
    """
    ...
    total_series = ct_series_count + mr_series_count

    # ── Generate unique dates for each modality (no overlap) ─────────────────
    all_dates = generate_study_dates(total_series, start_year=start_year)
    ct_dates  = sorted(all_dates[:ct_series_count])
    mr_dates  = sorted(all_dates[ct_series_count:])

    # ── Banner ────────────────────────────────────────────────────────────────
    print()
    print("╔══════════════════════════════════════════════════════════════════╗")
    print("║   Longitudinal Multi-Modality DICOM Phantom Generator  v1.0    ║")
    print("╚══════════════════════════════════════════════════════════════════╝")
    print()
    print(f"  Patient ID    : {patient_id}")
    print(f"  Patient Name  : {patient_name}")
    print(f"  CT  series    : {ct_series_count}  (AXIAL,    HU signed 16-bit)")
    print(f"  MR  series    : {mr_series_count}  (SAGITTAL, T1 unsigned 0-1023)")
    print(f"  Total series  : {total_series}")
    print(f"  Matrix size   : {image_size} × {image_size}")
    print(f"  Output dir    : {output_dir}/")
    print()
    print("  Study dates assigned:")
    for i, (d, t) in enumerate(ct_dates):
        print(f"    CT  session {i+1}  →  {d[:4]}-{d[4:6]}-{d[6:]}  {t[:2]}:{t[2:4]}")
    for i, (d, t) in enumerate(mr_dates):
        print(f"    MR  session {i+1}  →  {d[:4]}-{d[4:6]}-{d[6:]}  {t[:2]}:{t[2:4]}")
    print("─" * 68)

    series_global_idx = 0

    # ── CT series ─────────────────────────────────────────────────────────────
    for i, (study_date, study_time) in enumerate(ct_dates):
        series_global_idx += 1
        generate_modality_series(
            modality      = "CT",
            study_date    = study_date,
            study_time    = study_time,
            patient_id    = patient_id,
            ...
            series_index  = series_global_idx,
            total_series  = total_series,
            image_size    = image_size,
            num_slices    = slices_per_ct,
            slice_thickness = slice_thickness,
        )

    # ── MR series ─────────────────────────────────────────────────────────────
    for i, (study_date, study_time) in enumerate(mr_dates):
        series_global_idx += 1
        generate_modality_series(
            modality      = "MR",
            study_date    = study_date,
    ...
    print()
    print("═" * 68)
    print(f"  ✓  Done!  {total_series} series saved under '{output_dir}/'")
    print(f"     PatientID   : {patient_id}")
    print(f"     CT studies  : {ct_series_count}  │  MR studies : {mr_series_count}")
    print("═" * 68)
    print()
    print("  How to load in your viewer:")
    print("  ┌─────────────────────────────────────────────────────────────┐")
    print("  │  3D Slicer   : File → Add DICOM Data → load root folder    │")
    print("  │  Horos/OsiriX: File → Import → select root folder          │")
    print("  │  RadiAnt     : Open Folder → root folder                   │")
    print("  │  MicroDicom  : File → Open Folder                          │")
    print("  │                                                             │")
    print("  │  Viewer will auto-group series by PatientID + StudyDate.   │")
    print("  │  You should see ONE patient with multiple dated studies.   │")
    print("  └─────────────────────────────────────────────────────────────┘")
    print()
...
if __name__ == "__main__":
    create_longitudinal_study(
        output_dir      = "CT&MR",
        ct_series_count = 3,     # 3 CT scans on 3 different dates
        mr_series_count = 3,     # 3 MR scans on 3 different dates
        slices_per_ct   = 12,    # set None for random 10-15
        slices_per_mr   = 12,    # set None for random 10-15
        image_size      = 512,
        slice_thickness = 5.0,   # mm between slices
        start_year      = 2021,  # first possible study year
    )